...
class SmartGlassesGUI:
    def __init__(self, root):
        ...
        self.overlay_left = tk.Frame(self.left, bg="#000000")
        self.overlay_left.place(x=12, y=12)  # overlay sopra il canvas

        self.lbl_od = self._pill(self.overlay_left, "👁️ Object Detection: ON")
        self.lbl_od.pack(anchor="w", pady=(0, 6))

        self.lbl_inf = self._pill(self.overlay_left, "⏱️ Inference Time: 200.00 ms")
        self.lbl_inf.pack(anchor="w", pady=(0, 6))

        self.lbl_fps = self._pill(self.overlay_left, "⚡ FPS: 5 fps")
        self.lbl_fps.pack(anchor="w")

        # --- Badge in alto a destra (sopra al cap_right) ---
        self.overlay_right = tk.Frame(self.right, bg="#000000")
        self.overlay_right.place(relx=1.0, x=-12, y=12, anchor="ne")

        self.badge_batt = self._pill(self.overlay_right, f"🔋 {self.state.battery_pct}%", pad=(10, 4))
        self.badge_wifi = self._pill(self.overlay_right, "📶 ON", pad=(10, 4))
        self.badge_od = self._pill(self.overlay_right, "👁️ ON", pad=(10, 4))

        self.badge_batt.pack(side="left", padx=(0, 8))
        self.badge_wifi.pack(side="left", padx=(0, 8))
        self.badge_od.pack(side="left")
    ...
    def apply_state(self):
        self.lbl_od.config(text=f"👁️ Object Detection: {'ON' if self.state.object_detection_on else 'OFF'}")
        self.lbl_inf.config(text=f"⏱️ Inference Time: {self.state.inference_ms:.2f} ms")
        self.lbl_fps.config(text=f"⚡ FPS: {self.state.fps:.0f} fps")

        self.badge_batt.config(text=f"🔋 {self.state.battery_pct}%")
        self.badge_wifi.config(text=f"📶 {'ON' if self.state.wifi_on else 'OFF'}")
        self.badge_od.config(text=f"👁️ {'ON' if self.state.object_detection_on else 'OFF'}")
